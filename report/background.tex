...
caption={If- else if- else statement example corresponding to
Figure~\ref{fig:nested_ifs}.}]
if(smth){
	//do something 1
} else if(smth2){
	//do something 2
...
%	\noindent\begin{minipage}{0.48\textwidth}
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.75\textwidth]{figures/if_elseif_else_example}
		\caption{Minimal example of two nested $\gamma$-nodes representing the
the same control flow as the code in Listing~\ref{lst:nested_ifs}.}
