...
\textit{$\theta$}-nodes represent tail controlled loops. In C/C++ they are
equivalent to \textit{do-while loops} containing the representation of the body
of the loop. Like with the $\gamma$-node, its input is the predicate. All other
edges are the dependencies needed by its subgraph(s) representing the statements
...
subgraph in the subsection of the \textit{false} clause. The $\gamma$- and
$\theta$-nodes both need to have the exact same predicate, and same dependencies
on the predicate, if the combined subgraph is to represent a for-loop.
...
\todo[inline]{Perhaps put listing with the code that
Figure~\ref{fig:for_loop_rec_fib_print_ex} represents here? There should be
